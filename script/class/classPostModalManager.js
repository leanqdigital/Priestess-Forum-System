class PostModalManager {
  static async open(post) {
          ...
          <article class="flex-1 flex flex-col gap-3 w-full post bg-primary-100 rounded-lg shadow-sm max-[500px]:mb-[150px] ${
            ...
            <section id="modal-comments-section" class="max-h-[80vh] overflow-auto min-[1100px]:h-max ${
              ...
              <div class="comment-form flex min-[500px]:flex-col gap-3 w-full bg-primary-100 min-[500px]:rounded max-[500px]:p-4 z-[999] max-[500px]:fixed bottom-0 pb-6 right-0 w-full max-[500px]:bg-primary">
                <div id="comment-editor" class="comment-editor text-white p-2 h-[100px] w-full max-[500px]:h-10 border border-gray-600 rounded" contenteditable="true" placeholder="Write a comment..."></div>
                <button id="submit-comment" class="w-fit bg-secondary text-white px-4 py-2 rounded hover:bg-secondary-200 transition-colors">
                  Post
                </button>
              </div>
    ...
    // Open the modal (e.g. you might call modal.show() if using a library)
  ...
  static async loadComments(postId) {
    try {
      ...
      // For each comment, update its vote status individually using its own comment id.
      await Promise.all(
        comments.map(async (comment) => {
          ...
          comment.voteCommentCount = voteRecords.length; // assuming one vote per user per comment
        })
      );
